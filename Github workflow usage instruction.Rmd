...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
comp_normalized_rt10.1 <- as.data.frame(t(apply(df_step3[, 2:ncol(df_step3)],
                                                MARGIN = 1, 
...
## HCA
```{r, echo=FALSE, warning = FALSE, message=FALSE}
hc_df <- comp_normalized_rt10.1 %>% select(-c("Octane_rating", "sampling_season", "gas_station","Sample_name"))
...
features <- subset(df_step3, select = 5:ncol(df_step3))
...
pdb <- cbind(data.frame(tsne), df_step3$samp_event)
options(warn = -1)
tsne_plot <- plot_ly(data = pdb ,x =  ~X1, y = ~X2, z = ~X3, 
               color = ~df_step3$samp_event) %>% 
  add_markers(size = 8) %>%
  layout( 
    xaxis = list(
      zerolinecolor = "#ffff",
...
features <- comp_normalized_rt10.1 %>% select(-c("Octane_rating", "sampling_season", "gas_station","Sample_name"))

umap <- umap(features, n_components = 3)

layout <- cbind(data.frame(umap[["layout"]]), comp_normalized_rt10.1$gas_station)
