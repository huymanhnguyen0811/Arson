...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
count_ASTM <- function(data, which_step) {
  count <- 0
...
count_ASTM(bind_rows(df_list_step1.1), which_step = "STEP 1.1")
count_ASTM(bind_rows(list_remaining_area), which_step = "STEP 1.2")
count_ASTM(rt10.1, which_step = "STEP 1.3")
count_ASTM(comp_normalized_rt10.1, which_step = "STEP 2")
count_ASTM(shared_comp_rt10.1, which_step = "STEP 3")
```
...
PCAtools::biplot(p_rq1_rt10.1,
                 lab = NULL, # row.names(p_rq1_rt10.1$metadata),
                 colby = 'Fuel type',
                 hline = 0, vline = 0,
