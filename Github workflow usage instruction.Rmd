...
This repo is accompanying the publication: "..."

This workflow ran on Windows 11 OS 11th Gen Intel(R) Core(TM) i7-11800H @ 2.30GHz, 16 GB RAM; RStudio 2023.06.0+421 "Mountain Hydrangea" Release for Windows; R version 4.3.1
...
PCAtools::biplot(p_rq1_rt10.1,
                 lab = row.names(p_rq1_rt10.2$metadata),
                 colby = 'Fuel type',
                 hline = 0, vline = 0,
...
paste0("Number of significant compounds adjusted p-value < 0.1 = ", dim(rq1_p0.1)[1])
```
...
for (col in 3:ncol(df_stats_rq2_rt10.1)){
  pvalue.w_rq2[i] <- stats::wilcox.test(as.numeric(df_stats_rq2_rt10.1[which(df_stats_rq2_rt10.1$gas_station %in% c("Station_5", "Station_7", "Station_9")), col]),
                                        as.numeric(df_stats_rq2_rt10.1[which(df_stats_rq2_rt10.1$gas_station %in% c("Station_1", "Station_3", "Station_8")), col]))$p.value
  i <- i + 1
}
