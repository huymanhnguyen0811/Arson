...
for (row in 1:nrow(metadata)) {
  # If the compound has <= x % NA values, then take them 
...
summary_table <- metadata_na_remove %>% 
  ungroup() %>%
  rowwise() %>%
...
plotdf <- (summary_table %>% arrange(desc(mean_area), sd_area))[1:25,]
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
# Adding back the labelling
df_step3 <- as.data.frame(t(metadata_na_remove %>%  #metadata_na_remove
                              # filter(Feature %notin% unique(plotdf$Feature)) %>% # Remove compounds with high peak area
  ...
  mutate(supplier = ifelse(str_detect(Sample_name, "F001"), "misc",
                              ifelse(str_detect(Sample_name, "F002"), "misc",
                                     ifelse(str_detect(Sample_name, "F003"), "Shell",
                                            ifelse(str_detect(Sample_name, "F004"), "Cenovus",
                                                   ifelse(str_detect(Sample_name, "F005"), "misc",
...
for (r in 1:nrow(df_step3)) { 
  df_step3[r, which(base::is.na(df_step3[r,]))] <- as.list(runif(length(which(is.na(df_step3[r,]))),
                                                                 min = sort(df_all$Area)[1],
                                                                 max = sort(df_all$Area)[2]))
} 
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
percentage_normalized <- as.data.frame(t(apply(df_step3[, 6:ncol(df_step3)],
                                                MARGIN = 1, 
                                                function(row) {row/sum(row, na.rm = TRUE)}))) 


percentage_normalized <- percentage_normalized %>% 
  ...
  mutate(supplier = df_step3$supplier) %>%
  relocate(Octane_rating, sampling_season, gas_station, Sample_name, supplier, .before = 1)
```

### Log normalization
```{r, echo = FALSE, message = FALSE, warning = FALSE}
log_normalized_df <- as.data.frame(t(apply(df_step3[, 6:ncol(df_step3)], 1, function(x) log(x))))
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
df <- percentage_normalized %>%
  pivot_longer(cols = 6:ncol(.), names_to = "Feature", values_to = "Normalized_Area")

new_df <- left_join(df, metadata_na_remove %>% mutate(Feature = as.character(Feature)), by = "Feature")

focus_supplier <- "Suncor"
new_df <- new_df[, -c(9:ncol(new_df))] %>% 
  filter(supplier %in% focus_supplier)

new_df$Sample_name <- factor(new_df$Sample_name, levels = sort(unique(new_df$Sample_name)))

ggplot(data = new_df, aes(x = Sample_name, y = Normalized_Area, fill = Chemical_group)) +
  geom_bar(stat = "identity") +
  labs(title = focus_supplier, x = "Samples", y = "Percentage Normalized Peak Area (%)",
       fill = "Chemical Group") +
...
input <- percentage_normalized %>%
  filter(supplier %notin% c("misc", "unknown")) %>%
  filter(sampling_season %in% "blue")
...
factoextra::fviz_pca_biplot(res.pca,  
                            ...
                            repel = TRUE,
                            labelsize = 5, 
                            habillage = factor(input$supplier), # "sampling_season", "Octane_rating", "gas_station", "Sample_name", "supplier"
                            addEllipses = TRUE,
                            ellipse.level = 0.95,
