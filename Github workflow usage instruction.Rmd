...
for (c in 6:ncol(input)) {      
  ...
  for (col in 1:ncol(utils::combn(unique(input$gas_station), 2))) {  # "sampling_season", "Octane_rating", "gas_station"
    ...
    pval <- try(t.test(as.numeric(unlist(input[which(input$gas_station == p_1), c])), 
...
sig_comp_fillingNA0.001_TSN_normalized <- list()
...
for (m in pvaluecorrect) {
  ...
  sig_comp_fillingNA0.001_TSN_normalized[[j]] <- df %>%
          filter(., adjusted_pvalue < 0.05) %>%
          arrange(adjusted_pvalue)
  
  uncorrected_raw_pair_test_results[[j]] <- df
  
  if (dim(sig_comp_fillingNA0.001_TSN_normalized[[j]])[1] == 0) {
    next
  } else {
    ...
    minor <- c()
    compound_name <- c()
    for (row in 1:nrow(sig_comp_fillingNA0.001_TSN_normalized[[j]])) {
      # print(row)
      rt1 <- c(rt1, metadata[which(metadata$Feature == as.numeric(sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$Feature)),]$RT1)
      rt2 <- c(rt2,metadata[which(metadata$Feature == as.numeric(sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$Feature)),]$RT2)
      major <- c(major, metadata[which(metadata$Feature == as.numeric(sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$Feature)),]$Ion1)
      minor <- c(minor, metadata[which(metadata$Feature == as.numeric(sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$Feature)),]$Ion2)
    }
    
    sig_comp_fillingNA0.001_TSN_normalized[[j]]$RT1 <- rt1
    sig_comp_fillingNA0.001_TSN_normalized[[j]]$RT2 <- rt2
    sig_comp_fillingNA0.001_TSN_normalized[[j]]$Ion1 <- major
    sig_comp_fillingNA0.001_TSN_normalized[[j]]$Ion2 <- minor
    
    ## Get compound name 
    compound_name <- c()
    chem_grp <- c()
    for (row in 1:nrow(sig_comp_fillingNA0.001_TSN_normalized[[j]])) {
      idx <- which(abs(target_comp$RT1 - sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$RT1) <= 0.1 &
                     abs(target_comp$RT2 - sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$RT2) <= 0.1 &
                     abs(target_comp$Ion1 - sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$Ion1) <= 0.5 &
                     abs(target_comp$Ion2 - sig_comp_fillingNA0.001_TSN_normalized[[j]][row,]$Ion2) <= 0.5)
      # print(paste0(row, "_", idx))
      if (identical(idx, integer(0))) {
    ...
    sig_comp_fillingNA0.001_TSN_normalized[[j]]$compound_name <- compound_name
    sig_comp_fillingNA0.001_TSN_normalized[[j]]$Chemical_group <- chem_grp
    print(sig_comp_fillingNA0.001_TSN_normalized[[j]] %>%
            arrange(adjusted_pvalue))
...
ggplot(data = collapsed_df_station3467, aes(x = RT1, y = RT2, size = `TSN Normalized Area`, color = `Chemical_group`)) +
  geom_point(pch = 21) + 
  facet_wrap(~gas_station) +
  # scale_x_continuous(breaks = seq(from = 0, to = 40, by = 5)) +
  scale_size(limits = c(min(collapsed_df_station3467$`TSN Normalized Area`),
  ...
  guides(colour = guide_legend(override.aes = list(size=5))) +
  theme_classic(base_size = 15) +
  labs(x = "Retention time 1", y = "Retention time 2",  color = "Chemical Group") +
  theme(plot.title = element_text(hjust = 0.5)) +
  ...
  annotate("text", x = 6, y = 3.35, label = "1", 
           hjust = 0, vjust = 0.5, size = 5, color = "black", fontface = "bold") +
  # BTEX
  annotate("rect", xmin = 14, xmax = 17, ymin = 3.8, ymax = 4, 
           color = "red", fill = NA, size = 0.5)  +
  annotate("text", x = 13.5, y = 4, label = "2", 
           hjust = 0, vjust = 0.5, size = 5, color = "black", fontface = "bold") +
  # Castle Group
  annotate("rect", xmin = 18.5, xmax = 21.5, ymin = 3.8, ymax = 3.95, 
           color = "red", fill = NA, size = 0.5) + 
  annotate("text", x = 18, y = 3.95, label = "3", 
           hjust = 0, vjust = 0.5, size = 5, color = "black", fontface = "bold")
  

ggsave(filename = paste0("Bubble_plot","_only_non-uniquefillNA0.001_TSN_sig_comp_nonunique_unionize", format(Sys.time(), "%d-%m-%Y"),".png"), 
       plot = testbubble, 
       device = "png",
       dpi = 600,        # Set DPI to 600
...
combined_df <- bind_rows(sig_comp_fillingNA0.001_TSN_normalized[[1]] %>% dplyr::select(-c(pval, adjusted_pvalue)), 
                         sig_comp_fillingNA0.001_TSN_normalized[[2]] %>% dplyr::select(-c(pval, adjusted_pvalue)), 
                         sig_comp_fillingNA0.001_TSN_normalized[[3]] %>% dplyr::select(-c(pval, adjusted_pvalue)),
                         sig_comp_fillingNA0.001_TSN_normalized[[4]] %>% dplyr::select(-c(pval, adjusted_pvalue)),
                         sig_comp_fillingNA0.001_TSN_normalized[[5]] %>% dplyr::select(-c(pval, adjusted_pvalue)),
                         sig_comp_fillingNA0.001_TSN_normalized[[6]] %>% dplyr::select(-c(pval, adjusted_pvalue))) %>% distinct()
...
for (i in 1:length(percentage_normalized)) {
  ...
  final_vec <- Reduce(union, list(sig_comp_fillingNA0.001_TSN_normalized[[1]]$Feature, 
                                  sig_comp_fillingNA0.001_TSN_normalized[[2]]$Feature, 
                                  sig_comp_fillingNA0.001_TSN_normalized[[3]]$Feature, 
                                  sig_comp_fillingNA0.001_TSN_normalized[[4]]$Feature, 
                                  sig_comp_fillingNA0.001_TSN_normalized[[5]]$Feature, 
                                  sig_comp_fillingNA0.001_TSN_normalized[[6]]$Feature))
  
  pca_input <- p_nonunique_df %>% dplyr::select(-setdiff(colnames(p_nonunique_df[,6:ncol(p_nonunique_df)]), final_vec)) %>% filter(gas_station %in% "Station 1")
