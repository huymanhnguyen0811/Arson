...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
target_comp <- readxl::read_xlsx(path = paste0(getwd(), "/Shortened ILR Compound List PF001A Fixed.xlsx"))
...
for (i in 1:nrow(combi)) {
  for (j in 1:nrow(target_comp)) {
    ...
    df[nrow(df) + 1,] <- c(as.numeric(combi[i,][1]),
                           ...
                           100*(length(unique(temp[idx2,]$Sample_name)) / 71))
  }
}
...
for (j in 1:nrow(target_comp)) {
  ...
  colnames(summary_df)[i] <- paste0(target_comp[j,]$Compound, " (n = ", length(unique(temp[idx2,]$Sample_name)), ")")
  i <- i + 1 
}
...
summary_df2 <- data.frame(target=character(), missing_sample=character())
for (j in 1:nrow(target_comp)) {
  idx1 <- which((df_step2$Ion1 <= (target_comp[j,]$Ion1 + 0.1) & df_step2$Ion1 >= (target_comp[j,]$Ion1 - 0.1) & 
                   df_step2$Ion2 <= (target_comp[j,]$Ion2 + 0.1) & df_step2$Ion2 >= (target_comp[j,]$Ion2 - 0.1)) |
  ...
  if (length(unique(temp[idx2,]$Sample_name)) < 71) {
    ...
    print(ggplot(data = temp[idx2,], aes(x = Sample_name, y = Area, fill = Octane_rating)) + 
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
alignment <- grouping_comp_ver1(df_step2,
                             rt1thres = 0.4,
                             rt2thres = 0.16,
...
*** take  the peaks that was aligned to target compound list and try clustering.
