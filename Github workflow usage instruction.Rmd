...
## QUALITY CONTROL 1: Confirming ASTM at each step of Data processing

(UPDATE 22 April 2024:) We don't have ASTM compounds at this step any more, such as Octane, Nonane. Also, THe RT1 of another ASTM compounds "Benzene, propyl-" is ~19.04, which deviates drastically from the normal RT1 of this compound ~14.41 !!!!
...
hist((shared_comp_rt10.1 %>%
  filter(Area < 2000000))$Area)


df_step3 <- shared_comp_rt10.1 %>%
  # filter(Area < 2000000) %>%
  ...
  relocate(Octane_rating, samp_event, gas_station, Sample_name, .after = 1)
```
...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
## Filter out compounds that consistently appear in high values across majority of samples
...
column_sums <- colSums(df_step3[,4:ncol(df_step3)])
...
for (r in 1:nrow(df_step3)) { 
  df_step3[r, which(base::is.na(df_step3[r,]))] <- min(shared_comp_rt10.1$Area)
} 
```
...
```{r , echo=FALSE, warning = FALSE, message=FALSE}
for (r in 1:nrow(df_step3[, 3:ncol(df_step3)])) { 
  df_step3[, 3:ncol(df_step3)][r, which(base::is.na(df_step3[, 3:ncol(df_step3)][r,]))] <- 3*min(df_step3[, 3:ncol(df_step3)][r, which(!base::is.na(df_step3[, 3:ncol(df_step3)][r,]))])
} 
...
```{r , echo=FALSE, warning = FALSE, message=FALSE}
for (r in 1:nrow(df_step3)) { 
  df_step3[r, which(base::is.na(df_step3[r,]))] <- runif(length(which(is.na(df_step3[r,]))),
                                                         min = sort(as.numeric(unlist(df_step3[, 4:ncol(df_step3)])))[1],
                                                         max = sort(as.numeric(unlist(df_step3[, 4:ncol(df_step3)])))[2])
} 
```

# STEP 4: Data Normalization
```{r, echo = FALSE, message = FALSE, warning = FALSE}
for (i in 1:length(df_step3)) {
...
comp_normalized_rt10.1 <- comp_normalized_rt10.1 %>% 
  ...
  mutate(sampling_season = df_step3$samp_event) %>%
...
# Export file for ML
```{r, echo = FALSE, message = FALSE, warning = FALSE}
writexl::write_xlsx(comp_normalized_rt10.1, path = paste0(getwd(), "/python/arson.xlsx"))
```
...
res.pca <- FactoMineR::PCA(
  subset(df_step3, select = 5:ncol(df_step3)), # comp_normalized_rt10.1 %>% select(-c("sampling_season", "Octane_rating", "gas_station", "Sample_name")),
  scale.unit = TRUE,
  graph = FALSE)
...
factoextra::fviz_pca_biplot(res.pca,  
                            ...
                            repel = TRUE,
                            labelsize = 10, 
                            habillage = factor(df_step3$samp_event),
                            addEllipses = TRUE,
                            ellipse.level = 0.95,
...
## HCA
```{r, echo=FALSE, warning = FALSE, message=FALSE}
hc_df <- comp_normalized_rt10.1 %>%
  select(-c(sampling_season, Octane_rating, gas_station, Sample_name))
...
features <- subset(dat, select = -c(sampling_season, Octane_rating, gas_station, Sample_name))
...
pdb <- cbind(data.frame(tsne), comp_normalized_rt10.1$gas_station)
options(warn = -1)
tsne_plot <- plot_ly(data = pdb ,x =  ~X1, y = ~X2, z = ~X3, 
               color = ~comp_normalized_rt10.1$gas_station) %>% 
  add_markers(size = 8) %>%
  layout( 
    xaxis = list(
      zerolinecolor = "#ffff",
