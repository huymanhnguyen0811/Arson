...
### Subset compound that only appear in 1 gas_station/Octane_rating/supplier/sampling_season
Non-unique gas_station: 35 compounds
Non-unique Octane_rating: 47 compounds
Non-unique supplier: 52 compounds
Non-unique sampling_season: 56 compounds => Some compounds only have 4 obs (2 for each of the 2 product_name)

```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
unique_comp <- c()

transform_df2 <- df_step3 %>%
  pivot_longer(cols = 6:ncol(.), names_to = "Feature", values_to = "PA")

for (feature in unique(transform_df2$Feature)) {
  if (length(unique(transform_df2[which(transform_df2$Feature == feature & !is.na(transform_df2$PA)),]$gas_station)) < 2) {
    unique_comp <- c(unique_comp, feature)
  }
}

# A). dataset with non-unique compounds
new_df <- df_step3 %>% select(-unique_comp)

# B). dataset with only unique compounds
unique_df <- cbind(df_step3[, c(1,2,3,4,5)], df_step3 %>% select(unique_comp))
```
...
plot_list <- list()
for (i in 1:length(log_normalized)) {
  pca_input <-
    log_normalized[[i]]
