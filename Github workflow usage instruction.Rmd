...
```{r, echo = FALSE, message = FALSE, warning = FALSE}
count_ASTM <- function(data, ASTM, which_step) {
  count <- 0
...
count_ASTM(df_step1.1, ASTM, which_step = "STEP 1.1")
count_ASTM(df_step1.2, ASTM, which_step = "STEP 1.2")
count_ASTM(alignment, ASTM, which_step = "STEP 2")



# Overlay Scatter plot as Visual confirmation of appropriate RT windows by checking overlapping of RT1 and RT2 of aligned compounds and ASTMs 
plotly::ggplotly(ggplot(data = alignment, aes(x = RT1, y = RT2, label = Compound)) +
                   geom_point(pch = 1, size = 0.5) +
                   geom_point(data = ASTM, pch = 4, size = 2,
                              aes(x=RT1, y= RT2, 
                                  colour = Compound)) + 
                   xlim(5, 40))
...
df_step3 <- shared_comp_alignment %>% 
  filter(Feature %in% ASTM_comp$Feature) %>% # union(topsd$Feature, ASTM_comp$Feature)
  dplyr::select(
    Sample_name,
    Octane_rating,
    sampling_season,
...
factoextra::fviz_pca_biplot(res.pca,  
                            ...
                            repel = TRUE,
                            labelsize = 10, 
                            habillage = factor(input$sampling_season), # "sampling_season", "Octane_rating", "gas_station", "Sample_name"
                            addEllipses = TRUE,
                            ellipse.level = 0.95,
