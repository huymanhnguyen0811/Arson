...
ggplot(data = rq1_significant_wilcox_alpha0.1_df, aes(x = Percent_Area, fill = fuel_type)) +
  geom_histogram(alpha = 0.5,bins=100, position = "identity") + 
  facet_wrap(.~collapsed_compound) 

ggplot(data = rq1_nonsignificant_wilcox_alpha0.1_df, aes(x = Percent_Area, fill = fuel_type)) +
  geom_histogram(alpha = 0.5,bins=100, position = "identity") + 
  facet_wrap(.~collapsed_compound) 
