...
data_plot_post_removal <- list() # NOTE: Data sets are all heavy left-skewed
for (i in 1:length(list_remaining_area)) {
  filter_area <- list_remaining_area[[i]]
  data_plot_post_removal[[i]] <- ggplot(data = filter_area,
                                    aes(x = Area)) +
    geom_histogram(bins = 2000000/25000) +
    ggtitle(indi_IL_file_list[[i]]) +
    scale_x_continuous(limits = c(0, 2000000)) +
...
# Overlaying remaining and removed data points
ggplot(data = df_both) +
...
test <- copy(all_data_pre_norm_filter_area)
test$Compound <- stringi::stri_replace_all_regex(test$Compound, 
                                        pattern = c("\\#", "\\*", "\\(", "\\)",
                                                    "\\[", "\\]", "\\:", "\\>",
                                                    "\\<", "\\/", "\\±"),
                                        replacement = c("","","","","", 
...
# Identify linear/non-linear relationship in RT1 (&RT2) - predict the effect of RT1 and RT2 on count of compound
...
for (n in 1:length(list_remaining_area)) {
  ...
  for (i in 1:(dim(item)[[1]] - 1)) {
    dif1 <- base::abs(base::diff(c(item[i,]$RT1, item[i + 1,]$RT1)))
    dif2 <- base::abs(base::diff(c(item[i,]$RT2, item[i + 1,]$RT2)))
    dif_rt[[count]] <- list(dif1, dif2)
    count <- count + 1
  }

  rt_dif_list[[n]] <- dif_rt
  names(rt_dif_list)[n] <- paste0(unique(list_remaining_area[[n]]$sample_name), "_diff_rt1rt2")

  # plot_dif_rt1rt2[[count]] <- ggplot(x = RT1, y = RT2, data = item) %>%
  #   geom_point()
}

# Examine distribution of diff in rt1 and rt2 in each of these files
extract_rt1 <- c()
extract_rt2 <- c()
for (i in 1:length(rt_dif_list[[6]])) {
  extract_rt1 <- c(extract_rt1, as.numeric(rt_dif_list[[6]][[i]][1]))
  extract_rt2 <- c(extract_rt2, as.numeric(rt_dif_list[[6]][[i]][2]))
}

summary(extract_rt1)
summary(extract_rt2)

# Pearson Correlation Test & Fit linear regression to peak width vs. RT1 and RT2---------------------
...
for (i in 1:length(list_remaining_area)) {
  temp <- list_remaining_area[[i]] %>%
    ...
    relocate(peak_width, .after = RT2)
  # Pearson Correlation Test
  cor_list[unique(list_remaining_area[[i]]$sample_name)] <- list(cor(temp[, c('RT1', 'RT2', 'peak_width')]))
  # Testing for linear relationship between peak_width and RT1 / RT2
  lm_list[unique(list_remaining_area[[i]]$sample_name)] <- summary(lm(peak_width ~ RT1 + RT2, data = temp))$adj.r.squared
}

# View all correlation of all samples ==> No relation of peak_width to RT1 / RT2 in any of the sample
for (i in 1:length(cor_list)) {
  print(cor_list[i])
  print(paste0("Adjust r squared: ", lm_list[i]))
}
# No significant R-squared of any of the models
any(lm_list > 0.9)

# Plot Number of individual compounds against retention time (1,2) ----------------------
ggplot(data = list_remaining_area[[3]]  %>%
         mutate(peak_width = .$Area/.$Height)) + 
  geom_point(aes(x = RT1, y = RT2)) + # , color = peak_width, size = peak_width
  scale_colour_gradientn(colors=c("red", "yellow", "green")) +
  scale_x_continuous(breaks = seq(3.5, 50, 0.2)) +
  scale_y_continuous(breaks = seq(2.75, 6, 0.125)) +
...
# Interactive heatpmap with plotly-------------------------------
# plot_list <- list()
# i <- 1
# for (name in seq_along(unique(all_data_pre_norm_filter_area$sample_name))) {
#   subset <- all_data_pre_norm_filter_area %>%
#     filter(., sample_name %in% name) %>%
#     mutate(peak_width = .$Area/.$Height)
#   plot_list[[i]] <- plot_ly(data = subset, x = ~RT1, y = ~RT2, type = 'scatter', mode = 'markers', 
#                             color = ~peak_width, colors = 'Reds')
#   i <- i + 1
# }
# 
# fig
...
rt1_area <- lm(RT1 ~ Area, data = all_data_pre_norm_filter_area)
summary(rt1_area)

y_pred2 <- predict(rt1_area, newdata = rt1_area$model, interval = "prediction")
df <- data.frame(all_data_pre_norm_filter_area, y_pred2)

# Make a pretty plot showing raw data and model predictions
ggplot(df, aes(x = Area, y = RT1)) +
...
## Using ANCOVA???? ------------------------------------------------------------------------------
ancova <- stats::lm(RT1 ~ fuel_type + Area, data = all_data_pre_norm_filter_area)
# Make predictions
y_pred <- predict(ancova, interval = "prediction")

# Combine with original data
ancova_pred <- data.frame(all_data_pre_norm_filter_area, y_pred)

# Make a pretty plot showing raw data and model predictions
ggplot(ancova_pred, aes(x = Area, y = RT1, group = fuel_type)) +
  geom_point(aes(colour = fuel_type, fill = fuel_type), alpha = 0.75, size = 4) +
  geom_line( aes(y = fit, color = fuel_type), size = 1, alpha = .5) +
  geom_ribbon(aes(ymin = lwr, ymax = upr, color = fuel_type, fill = fuel_type),
              alpha = 0.25) +
  theme_bw() + 
  theme(panel.grid = element_blank())
...
# K-means clustering on RT1 and RT2 ----------------------------------------------------
for (i in 1:length(list_remaining_area)) {
  # Clustering based on chemical identity (instead of retention time) 
  x <- matrix(c(list_remaining_area[[i]]$Ion1, list_remaining_area[[i]]$Ion2), ncol = 2)
  set.seed(runif(1, min = 1, max = 5))
  km.out <- kmeans(x, 5, nstart = 100)
  plot(x, col = (km.out$cluster + 1),
       xlab = "Ion1", ylab = "Ion2", pch = 20, cex = 2, 
       main = unique(list_remaining_area[[i]]$sample_name))
  
}
# Mask Ion1, Ion2 cluster color onto RT1 and RT2 graphs --> examine cluster is correctly packed together in chromatograms or not?
# toggle with cluster number for Diesel vs. Gas... How many main chemical classes/groups (e.g., single aromatic, aliphatic, etc.) should be distinguish in Diesel vs Gas?
...
# -> if no, then edit window threshold at step 1.3B; test different window threshold

# Test collapsing compounds function with ASTM list to confirm reliability
# ASTM_grouped <- grouping_comp_ver1(ASTM_list %>%
#                                      arrange(RT1, RT2),
#                                    rt1thres = 0.2,
#                                    rt2thres = 0.125,
#                                    ion1thres = 0.05, # Ion 1 and 2 indicates molecular structure
#                                    ion2thres = 0.05)
...
#                                                   length(unique(all_data_pre_norm_grouped_filter_area$gas_station)))
# 
# similar_compounds_filter_area_stations <- all_data_pre_norm_grouped_filter_area[idx_list_filter_area_stations[[1]],] 
# other_compounds_filter_area_stations <- all_data_pre_norm_grouped_filter_area[idx_list_filter_area_stations[[2]],] 
# unique_compounds_filter_area_stations <- all_data_pre_norm_grouped_filter_area[idx_list_filter_area_stations[[3]],]

# Check peak area coverage of similar; other; and unique compounds
# sum(similar_compounds_filter_area$Area)/sum(all_data_pre_norm_grouped_filter_area$Area)
# sum(other_compounds_filter_area$Area)/sum(all_data_pre_norm_grouped_filter_area$Area)
# sum(unique_compounds_filter_area$Area)/sum(all_data_pre_norm_grouped_filter_area$Area)
...
# ggplot(data = shared_comp_samples,
#        aes(x = Area)) +
#   facet_wrap(~gas_station, scales = "free_y") + 
#   geom_histogram(bins = 120) + 
#   labs(x = "Area") +
#   ggtitle("Area distribution") +
#   theme(text = element_text(size = 20),
...
#   # filter data by indexing, ALWAYS DO THIS INSTEAD OF CREATE SUBSET DATAFRAME
#   idx <- which(grepl(comp_grp, shared_comp_normalized$collapsed_compound, fixed = TRUE))
#   
#   if (length(unique(shared_comp_normalized[idx,]$sample_name)) > 30) {
#     similar_compounds_normalized_idx <- c(similar_compounds_normalized_idx, idx)
#   }
# }
# 
# similar_compounds_normalized <- shared_comp_normalized[similar_compounds_normalized_idx,]

# to extract normalized values of other_compounds from shared_comp
# other_compounds_normalized_idx <- c()
# for (comp_grp in unique(shared_comp_normalized$collapsed_compound)) {
#   # filter data by indexing, ALWAYS DO THIS INSTEAD OF CREATE SUBSET DATAFRAME
#   idx <- which(grepl(comp_grp, shared_comp_normalized$collapsed_compound, fixed = TRUE))
#   
#   if (length(unique(shared_comp_normalized[idx,]$sample_name)) < 31 & length(unique(shared_comp_normalized[idx,]$sample_name)) > 1) {
#     other_compounds_normalized_idx <- c(other_compounds_normalized_idx, idx)
#   }
# }
# 
# other_compounds_normalized <- shared_comp_normalized[other_compounds_normalized_idx,]
