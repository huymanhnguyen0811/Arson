...
# GasData <- as.vector(t(mydata[,c(2:22)])) # 100149 data points
# DieselData <- as.vector(t(mydata[,c(23:26)])) # 19076 data point
# 
# sum(mydata[,c(2:22)] == 0) # Among 100149 data points in Gas, 91645 data points are NA (or 0)
# sum(mydata[,c(23:26)] == 0) # Among 19076 data points in Diesel, 11690 data points are NA (or 0)
# 
# # Histogram
# hist(GasData, col='steelblue', main='Normal')
# hist(DieselData, col='steelblue', main='Non-normal')
# 
# # Q-Q plots aka. Normal Probability plots
# stats::qqnorm(GasData, main='Normal')
# stats::qqline(GasData)
# 
# stats::qqnorm(DieselData, main='Non-normal')
...
# 
# # Levene’s test-non-normally distributed data (car::leveneTest()); p-val > 0.05 -> no significant difference between the tested sample variances.
# library(car)
...
# 
# # Fligner-Killeen test (fligner.test()) - p-value < 0.05, there is significant difference between tested sample variance
