...
# Figure 3. Box plot of chemical groups of from Figure 1 in different gas stations - Before versus After best imp/norm with 6 SUBPLOTS - Picking out 3 gas stations that have the best consistency in imp/norm
...
y <- textGrob("Gas Station", rot = 90, gp = gpar(fontsize = 20))
x <- textGrob("Proportion of total peak area (%)", gp = gpar(fontsize = 20))
...
ggsave(filename = paste0("figure3_Duel_horizontal-stacked-bar-plot_before and after imp-norm","_chemicalgroup_by gas station", format(Sys.time(), "%d-%m-%Y"),".png"), 
       plot = figure3, 
       device = "png",
       dpi = 600,        # Set DPI to 600
       width = 20,       # Width in inches (adjust as needed)
...
# ifelse(min_class_count_all > 1, min_class_count_all, 2)
K <- 3 # try K = 3 so that each fold has 1 sample of station 10 (which only have 3 samples) 
# K <- 10
...
repeats <- 5
K_folds <- createMultiFolds(data[[type_col]], k = K, times = repeats)

res_list_5repeated3fold <- vector("list", length = K * repeats * 4) 
conf_mat_list_5repeated3fold <- vector("list", length = K * repeats * 4) 
selected_features_5repeated3fold <- vector("list", length = K * repeats * 4) 
prob_matrix_5repeated3fold <- vector("list", length = K * repeats * 4) 
y_test_and_preds_list_5repeated3fold <- vector("list", length = K * repeats)
i <- 1
...
for(f in seq_along(K_folds)) {
  ...
  selected_features_5repeated3fold[[i]] <- X_train
  
  y_test_and_preds_list_5repeated3fold[[f]]$y_test <- y_test 
  y_test_and_preds_list_5repeated3fold[[f]]$all_feats_preds <- all_feats$preds
  
  prob_matrix_5repeated3fold[[i]] <- all_feats$p_m
  ...
  if(length(sig_feats)>0) {
    ...
    selected_features_5repeated3fold[[i]] <- sig_feats
    
    y_test_and_preds_list_5repeated3fold[[f]]$tmp_sig_preds <- tmp_sig$preds
    
    prob_matrix_5repeated3fold[[i]] <- tmp_sig$p_m
  ...
  selected_features_5repeated3fold[[i]] <- rfa_res$best_features_rf
  
  y_test_and_preds_list_5repeated3fold[[f]]$rfa_res_predictions <- rfa_res$predictions
  
  prob_matrix_5repeated3fold[[i]] <- rfa_res$prob_matrix
  ...
  selected_features_5repeated3fold[[i]] <- rfe_feats
  
  y_test_and_preds_list_5repeated3fold[[f]]$tmp_rfe_preds <- tmp_rfe$preds
  
  prob_matrix_5repeated3fold[[i]] <- tmp_rfe$p_m
  conf_mat_list_5repeated3fold[[i]] <- conf_mat_plot(y_test, 
                                      tmp_rfe$preds, 
...
summary_table <- results_df %>%
  ...
  summarise(
    ...
    MCC_mean = round(mean(MCC_Multiclass), 2),
    MCC_sd = round(sd(MCC_Multiclass), 2),
...
# boxplot of number of selected features by method
...
plot <- ggplot(results_df, aes(x=method, y=MCC_Multiclass)) +
  ...
  geom_signif(comparisons=list(c("RFA","All"),c("RFA","Pairwise"),c("RFA","RFE")), test=t.test, test.args = list(paired = F), map_signif_level=TRUE, step_increase=0.08) +
  theme_bw(base_size = 20) +
...
library(ggsignif)
library(ggpubr)
plot <- ggplot(results_df, aes(x=method, y=Accuracy)) +
  geom_boxplot() +
  stat_summary(fun.data = function(x) {
    stats <- boxplot.stats(x)$stats  # stats: lower whisker, Q1, median, Q3, upper whisker
    data.frame(ymin = stats[1], ymax = stats[1])
  ...
  geom_signif(comparisons=list(c("RFA","All"),c("RFA","Pairwise"),c("RFA","RFE")), test=t.test, test.args = list(paired = T), map_signif_level=TRUE, step_increase=0.08) +
  theme_bw(base_size = 20) +
  labs(x = "Feature selection methods",
       title="Accuracy across folds by method")
plot

# reshape to wide: one row per fold, columns = methods
wide_acc <- results_df %>%
  select(fold, method, MCC_Multiclass) %>%
  tidyr::pivot_wider(names_from = method, values_from = MCC_Multiclass)

stats::kruskal.test(list(wide_acc$All, wide_acc$Pairwise, wide_acc$RFA, wide_acc$RFE))
stats::wilcox.test(wide_acc$All, wide_acc$RFA)
...
$eval_metrics_rfe
$eval_metrics_rfe$Accuracy
Accuracy 
     0.7 

$eval_metrics_rfe$Kappa
    Kappa 
0.5774648 

$eval_metrics_rfe$AUC
[1] 0.8833333
...
$eval_metrics_rfa
$eval_metrics_rfa$Accuracy
Accuracy 
     0.8 

$eval_metrics_rfa$Kappa
    Kappa 
0.7260274 

$eval_metrics_rfa$AUC
[1] 0.9166667
...
$eval_metrics_sig
$eval_metrics_sig$Accuracy
Accuracy 
     0.6 

$eval_metrics_sig$Kappa
    Kappa 
0.4871795 

$eval_metrics_sig$AUC
[1] 0.8916667
...
$eval_metrics_full_features
$eval_metrics_full_features$Accuracy
Accuracy 
     0.4 

$eval_metrics_full_features$Kappa
    Kappa 
0.2105263 

$eval_metrics_full_features$AUC
[1] 0.8875
####----------------------------------------------------------------------
GAS STATION
...
sig_acc<- c()
rfa_acc<- c()
for (item in 1:length(rf_results_by_supplier_with_miscnunknown_list)) {
  full_features_acc <- c(full_features_acc, rf_results_by_supplier_with_miscnunknown_list[[item]]$full_features_acc)
  rfe_acc <- c(rfe_acc, rf_results_by_supplier_with_miscnunknown_list[[item]]$acc_rfe)
  gbfe_acc <- c(gbfe_acc, rf_results_by_supplier_with_miscnunknown_list[[item]]$acc_gbfe)
  sig_acc <- c(sig_acc, rf_results_by_supplier_with_miscnunknown_list[[item]]$acc_sig)
  rfa_acc <- c(rfa_acc, rf_results_by_supplier_with_miscnunknown_list[[item]]$acc_rfa)
}

# Make data frame of prediction accuracies
...
testaccuracy_boxplot <- ggplot(df_long, aes(x = Technique, y = Accuracy)) +
...
```{r , echo=FALSE, warning = FALSE, message=FALSE}
feature_df <- data.frame(Feature = rf_results_by_gas_station_with_miscnunknown_sig_rfa$rfa_selected_feats)
...
write_xlsx(feature_df, path = "RFA selected features_gas station classification.xlsx")
```
