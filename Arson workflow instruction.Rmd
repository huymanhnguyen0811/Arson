...
# write_xlsx(best_res$results_table %>% 
#              mutate(across(
#                .cols = where(is.numeric),
#                .fns  = ~ round(.x, 2)
#              )), path= paste0("imp_norm_result_table_", format(Sys.time(), "%d-%m-%Y"), ".xlsx"))
...
for(f in seq_along(K_folds)) {
  ...
  selected_features_5repeated3fold[[i]] <- colnames(X_train)
...
plot <- ggplot(results_df, aes(x=method, y=MCC_Multiclass)) +
  ...
  geom_signif(comparisons=list(c("RFA","All"),c("RFA","Pairwise"),c("RFA","RFE")), test=t.test, test.args = list(paired = F), map_signif_level=TRUE, step_increase=0.08, textsize = 7, vjust = 0.5) +
...
table(
  Actual    = as.character(y_test_and_preds_list_5repeated3fold[[15]]$y_test),
  Predicted = as.character(y_test_and_preds_list_5repeated3fold[[15]]$all_feats_preds)
)

# 2) Turn it into a data.frame for ggplot
cm_df <- read_xlsx(path = "cm_template.xlsx", sheet = 2)
...
table(
  Actual    = as.character(y_test_and_preds_list_5repeated3fold[[15]]$y_test),
  Predicted = as.character(y_test_and_preds_list_5repeated3fold[[15]]$tmp_sig_preds)
)

# 2) Turn it into a data.frame for ggplot
cm_df <- read_xlsx(path = "cm_template.xlsx", sheet = 3)
...
# RFE----------------------------
table(
  Actual    = as.character(y_test_and_preds_list_5repeated3fold[[14]]$y_test),
  Predicted = as.character(y_test_and_preds_list_5repeated3fold[[14]]$tmp_rfe_preds)
)

# 2) Turn it into a data.frame for ggplot
cm_df <- read_xlsx(path = "cm_template.xlsx", sheet = 4)
...
table(
  Actual    = as.character(y_test_and_preds_list_5repeated3fold[[15]]$y_test),
  Predicted = as.character(y_test_and_preds_list_5repeated3fold[[15]]$rfa_res_predictions)
)

# 2) Turn it into a data.frame for ggplot
cm_df <- read_xlsx(path = "cm_template.xlsx", sheet = 5)
