...
# QUALITY CONTROL OF STEP 1.2B: Plot number of peak remains after removal of limit observation with Percentage coverage after removal of limit observation (on top of each bar)
...
# Step 1: Remove features with values in only one row
...
## Option 2: LOD imputation 
## For each row, missing values in numeric columns (6:ncol) are replaced with 3 * (the lowest non-NA value in that row)
# impute_LOD <- function(df) {
#   df_out <- df
#   num_cols <- 6:ncol(df_out)
#   for (r in 1:nrow(df_out)) {
#     # extract numeric values in this row
#     row_vals <- as.numeric(df_out[r, num_cols])
#     missing_idx <- which(is.na(row_vals))
#     if (length(missing_idx) > 0) {
#       # only calculate minimum if there are non-missing values
#       if (sum(!is.na(row_vals)) > 0) {
#         min_val <- min(row_vals, na.rm = TRUE)
#         fill_val <- 3 * min_val
#       } else {
#         fill_val <- NA  # if the row is entirely NA (unlikely), you could choose a default value
#       }
#       row_vals[missing_idx] <- fill_val
#       df_out[r, num_cols] <- row_vals
#     }
#   }
#   return(df_out)
# }
      ...
      if(sum(!is.na(vec1)) >= 3 && sum(!is.na(vec2)) >= 3) {
        ...
        if(all(vec1 == vec1[1]) || all(vec2 == vec2[1])) {
          ...
          if(shapiro1 < 0.05 || shapiro2 < 0.05) {
            pval <- tryCatch(stats::wilcox.test(vec1, vec2)$p.value, error = function(e) NA)
          } else {
            pval <- tryCatch(stats::t.test(vec1, vec2)$p.value, error = function(e) NA)
          }
        }
      } else {
        next
...
evaluate_rf_test_accuracy <- function(data, 
                                      type_col, 
                                      remove_cols, 
                                      train_proportion,
  ...
  if(split_with_miscunknown_supplier) {
    cat("Splitting train/test with special condition for 'supplier'.\n")
    ## Here we use only samples that are not Misc or Unknown as training and testing on Misc + Unknown + small portion of non-Misc/unknown samples
    # Identify samples to force into the test set
    special_idx <- which(data[[type_col]] %in% excluding_supplier)
    
    # All other samples
    remaining_idx <- setdiff(seq_len(nrow(data)), special_idx)
    ...
    # Split the remaining samples according to train_proportion
    train_remaining <- createDataPartition(data[[type_col]][remaining_idx],
                                           p = train_proportion, list=FALSE)
    train_remaining_idx <- remaining_idx[train_remaining]
    test_remaining_idx  <- setdiff(remaining_idx, train_remaining_idx)

    # The test set is the union of the test_remaining and all the special samples
    test_idx <- c(test_remaining_idx, special_idx)

    X_train <- X[train_remaining_idx, ]
    X_test  <- X[test_idx, ]
    y_train <- y[train_remaining_idx]
    y_test  <- y[test_idx]
    y_train <- droplevels(y_train)
    # Drop unused factor levels from y_train so that it doesn't contain
    # levels "Miscellanous" or "unknown" that have no observations
  ...
  for(ntree in ntree_candidates){
    ...
    rf_fit <- caret::train(x = X_train,
                           ...
                           ntree = ntree,
                           importance = TRUE)
    current_best <- max(rf_fit$results[[metric]])
  ...
  final_rf <- best_model$finalModel
...
find_best_impute_normalize_new <- function(df,
  ...
  for(imp_name in names(imputation_methods)){
    ...
    for(norm_name in names(normalization_methods)){
      ...
      # 
      # test_acc <- evaluate_rf_test_accuracy(data = df_input_test_acc,
      #                                       type_col=type_col,
  ...
  # df_results$rank_corrScore  <- rank(-df_results$CorrScore,  ties.method = "min")
  # df_results$rank_KS       <- rank(-df_results$KS_p,       ties.method = "min")
  # df_results$rank_TestAcc    <- rank(-df_results$TestAcc,    ties.method = "min")
  ...
  ord <- order(
    ...
    -df_results$KS_p           # smaller rank_KS = better KS_p
  )
...
** right-subplot: bar plot with whisker of signal intensity of each chemical group in metadata
```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
...
p1 <- ggplot(chemical_group_counts, aes(x = reorder(Chemical_group, -Percentage), 
                                        y = Percentage, fill = Chemical_group)) +
...
## **Right subplot: Mean and standard deviation of values (from column 7 onward) ----------
...
p2 <- ggplot(data_long,
  ...
  labs(x = "Proportion of total signal intensity (%)", y = "Gas Station", fill = "Chemical Group") +
...
figure1 <- grid.arrange(p1, p2, ncol = 2, widths = c(1,2))
...
# Figure 2. Box plot of chemical groups of from Figure 1 in different gas stations - Before versus After best imp/norm with 6 SUBPLOTS - Picking out 3 gas stations that have the best consistency in imp/norm
...
ggsave(filename = paste0("Box_plot_NormalizedPA","_chemicalgroup_by gas station", format(Sys.time(), "%d-%m-%Y"),".png"), 
       plot = stauffer_boxplot2, 
       device = "png",
       dpi = 600,        # Set DPI to 600
       width = 20,       # Width in inches (adjust as needed)
       height = 8,       # Height in inches (adjust as needed)
...
run_rf_analysis_arson <- function(data,
                                  ...
                                  do_rfe           = FALSE,
                                  rfe_folds        = 5,
                                  do_gbfe          = FALSE,  # <--- New Arg for Greedy Backward Feature Elim
                                  do_sig           = FALSE,
                                  do_rfa           = FALSE,
  ...
  cat("Best Model Accuracy on training =", best_metric_val,
      " with ntree =", best_trees,
      " and mtry =", best_model$bestTune$mtry, "\n")
  
  # 7) Final model from best caret fit
  final_rf <- best_model$finalModel
  
  # 8) Probability predictions on the hold-out test (Full Feature Set)
  pred_prob_test <- predict(best_model, newdata=X_test, type="prob")
  
  # Add subcategory for reference
  new_pred_df_test <- as.data.frame(pred_prob_test)
  rownames(new_pred_df_test) <- rownames(X_test)
  
  # ---------- ADDITIONAL ACCURACY EVALUATION FOR FULL-FEATURE MODEL -----------
  # Get class predictions
  pred_class_test <- predict(best_model, newdata=X_test, type="raw") 
  acc_full <- mean(as.character(pred_class_test) == as.character(y_test))
  
  # --- Generate Heatmap (Full Features) ---
  {
    df_heatmap <- new_pred_df_test
    # row_names <- as.character(df_heatmap$gas_station)
  ...
  cat("\nTop Features by MeanDecreaseGini:\n")
  print(head(rf_importances))
  
  # 10) (Optional) RFE with fixed hyperparams
  final_rf_opt       <- NULL
  ...
  if(do_rfe){
    ...
    # Train final model on that subset
    X_train_opt  <- X_train[, rfe_selected_feats, drop=FALSE]
    final_rf_opt <- randomForest(
      x         = X_train_opt,
      y         = y_train,
      # ran with the same best mtry/ntree as the full features final model
      mtry      = best_model$bestTune$mtry,
      ntree     = best_trees,
      importance=TRUE
    )
    
    # ---------- ADDITIONAL ACCURACY EVALUATION FOR RFE MODEL -----------
    X_test_opt       <- X_test[, rfe_selected_feats, drop=FALSE]
    pred_class_opt   <- predict(final_rf_opt, newdata=X_test_opt, type="response")
    acc_rfe          <- mean(as.character(pred_class_opt) == as.character(y_test))

    # Probability predictions (RFE)
    pred_prob_test_opt <- predict(final_rf_opt, newdata=X_test_opt, type="prob")
    new_pred_df_test_opt <- as.data.frame(pred_prob_test_opt)
    rownames(new_pred_df_test_opt) <- rownames(X_test)
    # new_pred_df_test_opt$gas_station <- y_test
    
    # --- Generate Heatmap (RFE) ---
    {
      df_heatmap_opt <- new_pred_df_test_opt
      # row_names_opt <- as.character(df_heatmap_opt$gas_station)
      ...
      # df_heatmap_opt$gas_station <- NULL
  ...
  # 11) (Optional) Greedy Backward Feature Elimination (GBFE)
  ...
  if(do_gbfe){
    ...
    # We'll compare every reduced subset's test accuracy to the full-feature baseline:
    baseline_test_acc <- acc_full  # The test accuracy with ALL features
    ...
    while(improved && length(current_features) > 1){
      ...
      for(feature_to_remove in current_features){
        ...
        # Train a model using the trial subset
        set.seed(sample.int(999,1))
        ...
        # Check test accuracy with the trial subset
        pred_trial <- predict(trial_model, newdata=X_test[, trial_features, drop=FALSE], type="raw")
        trial_acc  <- mean(as.character(pred_trial) == as.character(y_test))
        
        # Remove the feature if test-accuracy is >= the full-feature baseline
        if(trial_acc >= baseline_test_acc){
          cat(sprintf(
            "Removing feature '%s' (test acc: %.3f >= baseline: %.3f)\n",
            feature_to_remove, trial_acc, baseline_test_acc
          ))
          # Update feature set
          current_features <- trial_features
          improved         <- TRUE
          break  # remove one feature at a time, then re-check
        }
      }
    }
    ...
    # Train final model with the reduced feature set
    final_rf_gbfe <- randomForest(
    ...
    pred_class_gbfe  <- predict(final_rf_gbfe, newdata=X_test_gbfe, type="response")
    acc_gbfe         <- mean(as.character(pred_class_gbfe) == as.character(y_test))
    
    # Probability predictions (GBFE)
    pred_prob_test_gbfe <- predict(final_rf_gbfe, newdata=X_test_gbfe, type="prob")
    new_pred_df_test_gbfe <- as.data.frame(pred_prob_test_gbfe)
    rownames(new_pred_df_test_gbfe) <- rownames(X_test)
    # new_pred_df_test_gbfe$gas_station <- y_test
    
    
    # --- Generate Heatmap (GBFE) ---
    {
      df_heatmap_gbfe <- new_pred_df_test_gbfe
      # row_names_gbfe <- as.character(df_heatmap_gbfe$gas_station)
      ...
      # df_heatmap_gbfe$gas_station <- NULL
      ...
      pheatmap(
        ...
        display_numbers = TRUE,
        number_format   = "%.2f" ,
        main = paste("Classification Probability (GBFE Features) -", sprintf("| Test Accuracy: %.2f", acc_gbfe)),
        breaks = seq(0, 1, length.out = 101)
      )
  ...
  # 12) (Optional) Significance-Based Feature Selection using pairwise tests
  final_rf_sig <- NULL
  sig_selected_feats <- NULL
  acc_sig <- NA
  
  if(do_sig){
    cat("\n========== BEGIN SIGNIFICANCE-BASED FEATURE SELECTION ==========\n")
    pairwise_results <- pairwise_significance_tests_modified(input_df = data,
                                                             group_col = group_for_significance,
                                                             start_col_index = start_col_index,
                                                             metadata = metadata,
                                                             target_comp = target_comp)
    index <- which.max(sapply(pairwise_results$corrected, function(subdf) length(unique(subdf$Feature))))
    sig_feats <- unique(pairwise_results$corrected[[index]]$Feature)
    cat("Number of significant compounds (before filtering):", length(sig_feats), "\n")
    # Ensure the significant features exist in X_train
    sig_feats <- intersect(sig_feats, colnames(X_train))
    cat("Number of significant compounds (after filtering):", length(sig_feats), "\n")
    
    if(length(sig_feats) == 0){
      cat("No significant compounds found in the features. Skipping significance-based selection.\n")
    } else {
      sig_selected_feats <- sig_feats
      set.seed(sample.int(999,1))
      sig_model <- caret::train(
        x = X_train[, sig_selected_feats, drop=FALSE],
        y = y_train,
        method = "rf",
        trControl = cv_ctrl, # use the same k-fold cv as the full features model
        # use best mtry and ntree from full features model
        tuneGrid = data.frame(mtry = best_model$bestTune$mtry), 
        ntree = best_trees,
        importance = TRUE
      )
      final_rf_sig <- sig_model
      
      pred_class_sig <- predict(sig_model, newdata=X_test[, sig_selected_feats, drop=FALSE], type="raw")
      acc_sig <- mean(as.character(pred_class_sig) == as.character(y_test))

      
      pred_prob_sig <- predict(sig_model, newdata=X_test[, sig_selected_feats, drop=FALSE], type="prob")
      new_pred_df_test_sig <- as.data.frame(pred_prob_sig)
      rownames(new_pred_df_test_sig) <- rownames(X_test)
      # new_pred_df_test_sig$gas_station <- y_test
      
      {
        df_heatmap_sig <- new_pred_df_test_sig
        # row_names_sig <- as.character(df_heatmap_sig$gas_station)
        # new_row_names_sig <- ave(row_names_sig, row_names_sig, FUN = function(x) {
        #   if(length(x) > 1) paste0(x, "_rep", seq_along(x)) else x
        # })
        # rownames(df_heatmap_sig) <- new_row_names_sig
        # df_heatmap_sig$gas_station <- NULL
        prob_matrix_sig <- as.matrix(df_heatmap_sig)
  ...
  # 13) (Optional) Recursive Feature Addition
  final_rf_rfa <- NULL
  recursive_selected_feats <- NULL
  acc_rfa <- NA
  
  if(do_rfa){
    cat("\n========== BEGIN RECURSIVE FEATURE ADDITION ==========\n")
    # Use the significant features from pairwise tests as the base features
    pairwise_results <- pairwise_significance_tests_modified(input_df = data,
                                                             group_col = group_for_significance,
                                                             start_col_index = start_col_index,
                                                             metadata = metadata,
                                                             target_comp = target_comp)
    base_features <- unique(pairwise_results$corrected[[1]]$Feature)
    X <- data %>% dplyr::select(-c(all_of(remove_cols), all_of(type_col)))
    y <- data[[type_col]]
    
    # base_features <- intersect(base_features, colnames(X_train))
    cat("Number of base features for recursive addition:", length(base_features), "\n")
    
    best_score_rf_list <- numeric()
    best_features_rf_list <- list()
    
    # using best mtry/ntree from full features model for RFA
    for(i in 1:100){
      result <- recursive_feature_addition(X, y, base_features = base_features,
                                           train_proportion=train_proportion,
                                           best_mtry=best_model$bestTune$mtry,
                                           best_ntree=best_trees)
      best_score_rf_list <- c(best_score_rf_list, result$best_score_rf)
      best_features_rf_list[[i]] <- result$best_features_rf
    }
    
    best_iter <- which.max(best_score_rf_list)
    recursive_selected_feats <- best_features_rf_list[[best_iter]]
    cat("Best Recursive Feature Addition Score:", best_score_rf_list[best_iter], "\n")
    cat("Selected Features from Recursive Addition:\n")
    print(recursive_selected_feats)
    
    # Ensure only valid features are used
    recursive_selected_feats <- intersect(recursive_selected_feats, colnames(X_train))
    
    set.seed(sample.int(999,1))
    recursive_model <- caret::train(
      x = X_train[, recursive_selected_feats, drop=FALSE],
      y = y_train,
      method = "rf",
      trControl = cv_ctrl,
      tuneGrid = data.frame(mtry = best_model$bestTune$mtry),
    ...
    final_rf_rfa <- recursive_model
    
    pred_class_recursive <- predict(recursive_model, newdata=X_test[, recursive_selected_feats, drop=FALSE], type="raw")
    acc_rfa <- mean(as.character(pred_class_recursive) == as.character(y_test))

    
    pred_prob_recursive <- predict(recursive_model, newdata=X_test[, recursive_selected_feats, drop=FALSE], type="prob")
    new_pred_df_test_recursive <- as.data.frame(pred_prob_recursive)
    rownames(new_pred_df_test_recursive) <- rownames(X_test)
    # new_pred_df_test_recursive$gas_station <- y_test
    
    {
      df_heatmap_recursive <- new_pred_df_test_recursive
      # row_names_recursive <- as.character(df_heatmap_recursive$gas_station)
      # new_row_names_recursive <- ave(row_names_recursive, row_names_recursive, FUN = function(x) {
      #   if (length(x) > 1) paste0(x, "_rep", seq_along(x)) else x
      # })
      # rownames(df_heatmap_recursive) <- new_row_names_recursive
      # df_heatmap_recursive$gas_station <- NULL
      prob_matrix_recursive <- as.matrix(df_heatmap_recursive)
      
      pheatmap(
        prob_matrix_recursive,
        fontsize = 20,
        fontsize_number = 30,
        cluster_rows    = FALSE,
        cluster_cols    = FALSE,
  ...
  result_list <- list(
    ...
    final_rf_all_feats    = final_rf,
    full_features_acc     = acc_full,
    rfe_model             = rfe_results_fixed,
    final_rf_opt          = final_rf_opt,
    rfe_selected_features = rfe_selected_feats,
    gbfe_model            = final_rf_gbfe,
    gbfe_selected_feats   = gbfe_selected_feats,
    prob_matrix = prob_matrix
  )
  
  if(do_sig){
    result_list$sig_model    <- final_rf_sig
    result_list$sig_selected_feats <- sig_selected_feats
    result_list$acc_sig <- acc_sig
    result_list$prob_matrix_sig <- prob_matrix_sig
  }
  
  if(do_rfa) {
    result_list$prob_matrix_opt    <- prob_matrix_opt
    result_list$acc_rfa  <- acc_rfa
     result_list$recursive_selected_feats <- recursive_selected_feats
  } 
  if(do_rfe) {
    result_list$prob_matrix_opt    <- prob_matrix_opt
  ...
  if(do_gbfe) {
    result_list$prob_matrix_gbfe    <- prob_matrix_gbfe
    result_list$acc_gbfe  <- acc_gbfe
  }
...
## Figure 4: Boxplot of test accuracy across five feature selection methods
```{r , echo=FALSE, warning = FALSE, message=FALSE}
...
sig_acc<- c()
rfa_acc<- c()
for (item in 1:length(rf_results_by_gas_station_with_miscnunknown_list)) {
  full_features_acc <- c(full_features_acc, rf_results_by_gas_station_with_miscnunknown_list[[item]]$full_features_acc)
  rfe_acc <- c(rfe_acc, rf_results_by_gas_station_with_miscnunknown_list[[item]]$acc_rfe)
  gbfe_acc <- c(gbfe_acc, rf_results_by_gas_station_with_miscnunknown_list[[item]]$acc_gbfe)
  sig_acc <- c(sig_acc, rf_results_by_gas_station_with_miscnunknown_list[[item]]$acc_sig)
  rfa_acc <- c(rfa_acc, rf_results_by_gas_station_with_miscnunknown_list[[item]]$acc_rfa)
}


df <- data.frame("Full_features" = full_features_acc,
                 "RFE" = rfe_acc,
                 "GBFE" = gbfe_acc,
                 "Pair_wise_test" = sig_acc,
                 "RFA" = rfa_acc)

# Reshape the data from wide to long format
df_long <- reshape2::melt(df, 
                measure.vars = c("Full_features", "RFE", "GBFE", "Pair_wise_test", "RFA"),
...
gasstation_testaccuracy_boxplot <- ggplot(df_long, aes(x = Technique, y = Accuracy)) +
  ...
  scale_x_discrete(labels = c("Full_features" = "Original features",
                              "RFE" = "RFE",
                              "GBFE" = "GBFE",
                              "Pair_wise_test" = "Pair-wise test",
                              "RFA" = "RFA")) +
  theme_minimal(base_size = 30) + 
  theme(axis.text.x = element_text(size = 30))

ggsave(filename = paste0("Figure 4_box_plot_test accuracy across five feature selection methods_", "supplier_", format(Sys.time(), "%d-%m-%Y"),".png"), 
       plot = supplier_testaccuracy_boxplot, 
       device = "png",
...
# Kruskal-Wallis Rank Sum Test - Is there different in Accuracy between these 5 feature selection methods
stats::kruskal.test(list(full_features_acc, rfe_acc, gbfe_acc, sig_acc, rfa_acc)) #=> there is no differences at all
## => there is no differences at all: Kruskal-Wallis chi-squared = 3.1759, df = 4, p-value = 0.5288
```

### Find out identity of sig_selected_features
```{r , echo=FALSE, warning = FALSE, message=FALSE}
feature_df <- data.frame(Feature = rf_results_by_gas_station_with_miscnunknown_list[[5]]$sig_selected_feats)
...
```{r class.source = 'fold-hide', echo = FALSE, message = FALSE, warning = FALSE}
recursive_feature_addition <- function(X, y, base_features, train_proportion,
                                       best_mtry, best_ntree, base_feature_accuracy) {
  
  # Ensure base features are included initially and keep data frame structure
  base_features_train <- X[, base_features, drop = FALSE]
  base_features_test  <- X[, base_features, drop = FALSE]
  
  # Define remaining features (those not in base_features)
  remaining_feature_names <- setdiff(colnames(X), base_features)
  remaining_features <- X[, remaining_feature_names, drop = FALSE]
  
  best_score_rf <- base_feature_accuracy
  best_features_rf <- base_features
  
  while (ncol(remaining_features) > 0) {
    # Combine the current base features with the remaining candidate features
    all_features_train <- cbind(base_features_train, remaining_features)
    
    # Train random forest on the combined feature set
    rf_all <- randomForest(x = all_features_train, 
                           y = as.factor(y_train), 
                           mtry      = best_mtry,
                           ntree     = best_ntree,
                           importance=TRUE)
    
    # Get importance scores (mean decrease in node impurity)
    importance_scores <- importance(rf_all, type = 2)
    
    # Extract importance scores for only the candidate features
    importance_remaining <- importance_scores[rownames(importance_scores) %in% colnames(remaining_features), ]
    
    # Rank remaining features by importance
    ranked_features <- names(sort(importance_remaining, decreasing = TRUE))
    
    # Safety check: break if no candidate feature is selected
    if (length(ranked_features) == 0) break
    
    # Select the candidate feature with highest importance
    best_next_feature <- ranked_features[1]
    
    # Check if the feature is already present (it shouldn't be, but this is a safeguard)
    if (!(best_next_feature %in% colnames(base_features_train))) {
      # add that highest important feature to the base feature
      base_features_train_temp <- cbind(base_features_train, remaining_features[, best_next_feature, drop = FALSE])
      base_features_test_temp  <- cbind(base_features_test, X_test[, best_next_feature, drop = FALSE])
    } else {
      warning("Feature ", best_next_feature, " is already present in base_features_train. Skipping addition.")
    }
    
    
    
    # Train the updated model with the new feature added
    rf_updated <- randomForest(x = base_features_train_temp, 
                               y = as.factor(y_train), 
                               mtry      = best_mtry,
                               ntree     = best_ntree,
                               importance=TRUE)
    ...
    pred_class_test <- predict(best_model, newdata=X_test, type="raw") 
    acc_new <- mean(as.character(pred_class_test) == as.character(y_test))
  
    y_pred_rf <- predict(rf_updated, base_features_test_temp)
    score_rf <- mean(y_pred_rf == y_test)
    
    # Add the new important feature to base feature Remove the selected feature from the candidate set
    remaining_features <- remaining_features[, !(colnames(remaining_features) %in% best_next_feature), drop = FALSE]
    
    # Update the best score and feature set if the new score improves
    if (score_rf > best_score_rf) {
      best_score_rf <- score_rf
      best_features_rf <- colnames(base_features_train)
    }
  }
  
  return(list(best_score_rf = best_score_rf, best_features_rf = best_features_rf))
}
