...
find_best_impute_normalize_new <- function(df,
  ...
  for(imp_name in names(imputation_methods)){
    ...
    for(norm_name in names(normalization_methods)){
      ...
      acc_kappa_auc <- evaluate_rf_performance(data = df_input_test_acc,
                                               type_col=type_col,
...
tune_rf_subset <- function(X_train, y_train, X_test, y_test,
  ...
  eval_metrics_res <- eval_metrics(true_labels = y_test, 
  ...
  return(list(model    = best_mod,
              ...
              p_m = prob_matrix,
              preds = preds,
              eval_metrics = eval_metrics_res,
              Accuracy = eval_metrics_res$Accuracy,
              Kappa = eval_metrics_res$Kappa,
              AUC = eval_metrics_res$AUC,
              F1_Weighted = eval_metrics_res$F1_Weighted,
              MCC          = eval_metrics_res$MCC_Multiclass
  ))
}
