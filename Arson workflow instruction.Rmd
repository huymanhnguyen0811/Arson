...
K <- 10 # ifelse(min_class_count_all > 1, min_class_count_all, 2)
# stratified folds on the full target vector
K_folds <- createFolds(data[[type_col]], k = K, returnTrain = TRUE)

res_list <- vector("list", length = K * 4) 
conf_mat_list <- vector("list", length = K * 4) 
i <- 1
...
for(f in seq_along(K_folds)) {
  ...
  conf_mat_list[[i]] <- conf_mat_plot(y_test, all_feats$preds, 
  ...
  if(length(sig_feats)>0) {
    ...
    conf_mat_list[[i]] <- conf_mat_plot(y_test, tmp_sig$preds, 
  ...
  conf_mat_list[[i]] <- conf_mat_plot(y_test, 
                                      rfa_res$predictions, 
